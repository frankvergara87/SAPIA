...
{	
	var pos_usc;
	if(ddlDep.id.indexOf("Usc_direccion") != -1)
	{
		pos_usc = ddlDep.name;
...
{
	var pos_usc;
	if(ddlProv.id.indexOf("Usc_direccion") != -1)
	{
...
function mostrarUbigeo(ddldist)
{
	...
	if(ddldist.id.indexOf("Usc_direccion") != -1)
	{
	...
	provincia = document.getElementById(pos_usc + 'ddlProvincia');	
	//ubigeo = departamento[departamento.selectedIndex].text + '-' + provincia[provincia.selectedIndex].text + '-' + ddldist[ddldist.selectedIndex].text;
...
function sinNumero(chk){	
	var pos_usc;
	if(chk.id.indexOf("Usc_direccion") != -1)
	{
		pos_usc = chk.name;
		pos_usc = pos_usc.split(':')[0] + "_";
	}
	if (chk.checked == true){			
		...
		setEnabled(pos_usc + 'ddlEdificacion', true, 'clsSelectEnable0');
		...
		setEnabled(pos_usc + 'ddlEdificacion', false, 'clsSelectDisable');
...
{
	var pos_usc;
	if(txt.id.indexOf("Usc_direccion") != -1)
	{
		pos_usc = txt.name;
		pos_usc = pos_usc.split(':')[0] + "_";
	}
...
{
	var pos_usc;
	if(idusc.id.indexOf("Usc_direccion") != -1)
	{
		pos_usc = idusc.name;
		pos_usc = pos_usc.split(':')[0] + "_";
	}
...
function onchange_edificacion(cbo){
	var pos_usc;
	if(cbo.id.indexOf("Usc_direccion") != -1)
	{
		pos_usc = cbo.name;
		pos_usc = pos_usc.split(':')[0] + "_";
	}
